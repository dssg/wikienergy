...
trans_params = [pymc.Beta('trans_params{}'.format(i), alpha=1,beta=1) for i in range(n_modes)]
...
for i in xrange(n_gamma):
    ...
    prob1 = trans_params[i*2]
    prob2 = trans_params[i*2+1]
    @pymc.deterministic
    def params(a=prob1,b=prob2):
        return np.array([a,b])
